...
- Renamed "Real-time telemetry pipelines" to **Machine Telemetry & Fault Visibility** and refocused it on the device edge (MQTT topic taxonomy, connectivity/heartbeat monitoring, Kafka consumers, normalizing vendor-specific machine output into one event shape). The 1M+ events and 100+ DynamoDB tables figures moved onto UDEP, and the state engine / utilization / idle-detection claims stay on Fleet Analytics where the resume puts them — so no two cards repeat each other.
- Added a **"one claim, one card"** rule to CONTENT.md with a claim-ownership table, so metrics and named mechanisms can't drift back into multiple cards. Shared technology names may recur; shared claims may not.
